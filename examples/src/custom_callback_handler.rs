...
async fn main() {
    ...
    //prints the received message to the console. Need to parse, and return this as JSON.
    let (ctag, mut rx) = channel
        .basic_consume_rx(args)
        .await
        .unwrap();
    tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            println!("{}", std::str::from_utf8(&msg.content.unwrap()).unwrap())
        }
    });
    //////////////////////////////////////////////////////////////////////////////
    // publish message
